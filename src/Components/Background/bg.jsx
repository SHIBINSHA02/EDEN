...
export const Bg = ({ width = 600, height = 400 }) => {
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uPlusSigns: {
          value: plusSigns.map(
            (sign) =>
              new THREE.Vector3(sign.x, sign.y, sign.type === "primary" ? 1 : 0)
          ),
        }, // Ensure valid Vector3
        uPrimaryCenter: { value: new THREE.Vector3(...primaryCenter) },
        uPrimarySurround: {
          value: primarySurround.map((c) => new THREE.Vector3(...c)),
        },
    ...
    const handleResize = () => {
      ...
      material.uniforms.uPlusSigns.value = newPlusSigns.map(
        (sign) =>
          new THREE.Vector3(sign.x, sign.y, sign.type === "primary" ? 1 : 0)
      ); // Ensure valid Vector3
